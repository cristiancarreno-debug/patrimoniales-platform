 ...
 */
export function ModuleIframe({ title, src }: ModuleIframeProps) {
  ...
  return (
    <div className={`flex flex-col ${isFullscreen ? 'fixed inset-0 z-50 bg-surface-secondary p-4' : 'space-y-4'}`}>
      ...
      {/* iframe container — 100% ancho */}
      <div className={`relative bg-white rounded-lg border border-border overflow-hidden ${isFullscreen ? 'flex-1' : 'min-h-[calc(100vh-180px)]'}`}>
        {isLoading && (
